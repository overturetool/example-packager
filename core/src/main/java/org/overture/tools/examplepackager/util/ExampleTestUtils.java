...
public abstract class ExampleTestUtils {
	...
	/**
	 * Get sources for all the Overture public examples. Currently, only examples
	 * that are expected to parse and TC are returned. In other words, the examples
	 * with purposeful errors are not checked at the moment. Further work
	 * is needed on these.
	...
	private static Collection<ExampleTestData> getSubSources(
		...
		for (File f : dir.listFiles()) {
			...
			if (f.isDirectory()) {
				...
				if (!p.getSettings().getExpectedResult()
						.equals(ResultStatus.NO_CHECK)) {
					...
					r.add(new ExampleTestData(p.getSettings().getName(),
							dialect, source.toString()));
				}
			}
		}
